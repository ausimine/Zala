    ...
    def transfer(self, conn, is_serveur):
        try:
            while True:
                ...
                if is_serveur:
                    # Données venant du serveur, on envoie à tous les clients
                    for client in self.clients[:]:  # Copie de la liste pour éviter les problèmes de concurrence
                        try:
                            client.sendall(data)
                        except:
                            self.clients.remove(client)
                else:
                    # Données venant d'un client, on envoie à tous les serveurs
